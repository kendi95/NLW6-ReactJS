...
export const Room: FC = () => {
  ...
  const handleLikeQuestion = async(question_id: string, likeId: string | undefined) => {
    if (likeId) {
      await database.ref(
        `rooms/${room_id}/questions/${question_id}/likes/${likeId}`
        ).remove()
    } else {
      await database.ref(
        `rooms/${room_id}/questions/${question_id}/likes`
        ).push({ author_id: user?.id })
    }
  }
